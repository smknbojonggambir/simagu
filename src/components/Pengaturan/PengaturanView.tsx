  ...
  const handleRestoreFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (content) {
        ...
        if (success) {
          toast.success('Pemulihan database berhasil! Halaman akan dimuat ulang untuk memperbarui data.');
          setTimeout(() => {
            if (typeof window !== 'undefined') window.location.reload();
          }, 1000);
        } else {
          toast.error('Format file JSON backup tidak valid. Silakan gunakan file cadangan resmi SIMAGU.');
        }
      }
    };
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            if (file.size > 2 * 1024 * 1024) {
                              toast.error('Ukuran file logo terlalu besar. Harap gunakan gambar di bawah 2MB.');
                              return;
                            }
                            const reader = new FileReader();
                            reader.onloadend = () => {
